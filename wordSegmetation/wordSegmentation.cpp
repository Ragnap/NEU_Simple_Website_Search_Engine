...
const char* const OUTPUT_TEMP_INDEX_PATH = "./simple_website_search_engine/tempIndex/tempIndex";  //临时索引文件夹路径
...
int main() {
    ...
    while(1) {
        ...
        if(pageID % PAGE_PER_FILE == 0) {  //根据前面定义的宏确定一个文件中的网页个数
            outTempIndex.close();
            string targetFile = OUTPUT_TEMP_INDEX_PATH + to_string(pageID / PAGE_PER_FILE) + ".txt";
